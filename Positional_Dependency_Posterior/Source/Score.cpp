...
Score::Score(Alignment& alg){
	PRECISION = .5;	// the default value of PRECISION
	LAMBDA = .5;
	alignment = alg;
	alpha_exponent = 1.;
	logR = calculate_logR();
...
//        	std::vector <std::vector<double> > temp = shift(logR, rho);
//			cout << rho << '\t' << alpha << endl;
//			cout << rho << '\t' << alpha<< '\t'<<temp[3][6] << '\t'<<laplacian_of(re_scale(temp, alpha))[3][6] << '\t'<< logR[3][6] << endl;
...
//        cout << laplacian_determinant << endl;
	}else{

//		cout << "rho value is: " << parameters.give_rho() << endl;

//		std::vector <std::vector<double> > temp = shift(logR,parameters.give_rho());
		alpha_exponent = find_alpha_exponent(logR);
		rescaled_R = re_scale(logR, alpha_exponent);
//		std::vector <std::vector<double> > temp = shift(rescaled_R,parameters.give_rho());
//		cout << calculate_determinant(laplacian_of(temp), temp.size()-1) << endl;
//		cout << alpha_exponent << endl;
//	for(int i=0;i<rescaled_R.size();i++){
//		for(int j=0;j<=i;j++){
//			cout << j << '\t' << i <<'\t' << shift(rescaled_R,parameters.give_rho())[i][j] << '\t' << logR[i][j]<< endl;
//		}
//	}
		laplacian_determinant = calculate_determinant(laplacian_of(shift(rescaled_R,parameters.give_rho())), rescaled_R.size()-1);
//		cout << exp(laplacian_determinant) << endl;
	}
//	for(unsigned int i=0; i<logR.size(); i++){
//		for(unsigned int j=0;j<i;j++){
////			cout << j << '\t' << i << '\t' << (posterior(i,j)) << endl;
//			cout << "logR\t" << logR[i][j] << endl;
////			posterior(i,j);
//		}
//	}
}
...
//	    logP_indep = 2*(lgamma(16*LAMBDA)-lgamma(alignment.nrows()+16*LAMBDA));
		logP_indep = 0.0;
...
//		    cout << alignment.get_n_i(alpha, j) << ", ";
...
//		      cout << i<< ' '<<j<<' '<<' ' << alpha << ' ' << beta << ' '<< alignment.get_n_ij(alpha,beta,i,j) << '\n';
		    }
		  }
//		  cout << logP_dep << "  " << logP_indep <<'\n';
...
//			difference = logR[l][k] + log(rho) - log(1.- rho);
//			if(difference > 0.){
//				M[l][k] = logR[l][k] + log(rho);
//			} else {
//				M[l][k] = log(1. - rho);
//			}
//			M[l][k] += log(1 + exp(-fabs(difference)));
...
//	}
//
//	for(unsigned int i = 0; i < alignment.ncols() ; i++){	// shifting logR entries by the min value which was found while setting the values of logR
//		for(unsigned int j = 0; j < i; j++){
//			if (logR[i][j] < min)
//				min = logR[i][j];
//		}
...
//	for(unsigned int i = 0; i < alignment.ncols() ; i++){	// shifting logR entries by the min value which was found while setting the values of logR
//		for(unsigned int j = 0; j < i; j++){
//			M[j][i] = M[i][j] = logR[i][j] - min;
//		}
//	}
//
////  now subtract the average row and column logR from each entry
//	  double num_non_diagonal = (double) (alignment.ncols() - 1);	// number of non-diagonal elements in each row/column
//	  double num_under_diagonal = ((double) alignment.ncols()*(alignment.ncols() - 1)) / 2.;	// number of elements under the main diagonal, like a triangle under main diagonal
//	  double tot_average = 0.;
//	  double correction[alignment.ncols()];
//	  for (unsigned int l=0; l < alignment.ncols() ;l++){
//		  correction[l] = .0;
//		  for (unsigned int k=0; k < alignment.ncols() ;k++){
////			  if(k!=l){
//			  correction[l] += M[l][k];
////			  }
//			  if (k < l){
...
//			  }
//		  }
//	    correction[l] /= num_non_diagonal;
//	  }
//	  tot_average /= num_under_diagonal;
//	  for (unsigned int l=0 ; l < alignment.ncols() ;l++){
//	    for (unsigned int k=0; k < l ; k++){
//	      M[k][l]-= (correction[l]*correction[k]) / tot_average;
//	      M[l][k] = M[k][l];
//	     }
//	  }
...
//	if (alpha!=1.0){
//		cout << "The value of alpha_exponent is: " << alpha << endl;
//	}
	return alpha;
}
 ...
 * */
	...
	for (unsigned int i = 0; i < logR.size(); i++){
		for(unsigned int j = 0; j < i; j++){
//			M[i][j] = transf(logR[i][j]/LOG_10, min, max, alpha_exponent);	// Lukas's version
...
//      cout << parameters.give_rho() << endl;
    ...
    posterior = log(shift(rescaled_R, parameters.give_rho())[i][j] - 1 + parameters.give_rho()) + numerator_determinant;
     // cout << i << '\t' << j << '\t' << log(shift(re_scale(logR, alpha_exponent), parameters.give_rho())[i][j]) << endl;
    // cout << i << '\t' << j << '\t' << numerator_determinant << '\t' << log(shift(rescaled_R, parameters.give_rho())[i][j] - 1 + parameters.give_rho()) << '\t' << laplacian_determinant << endl;
...
double Score::score_of_sequence(string seq){
	...
	//std::vector <std::vector<double> > R_new = shift(re_scale(temp, find_alpha_exponent(temp)), parameters.give_rho());
...
//	cout << seq << '\t' << determinant_new_R << '\t' << laplacian_determinant << '\t' << exp(determinant_new_R-laplacian_determinant) << endl;
	//	score -= background.uniform_background(seq.size());	// here I just used the simplest kind of background model, which is uniform and in log scale
...
//	double min = logR_new[0][1];	// initializing min
//	for(unsigned int i = 0; i < alignment.ncols() ; i++){	// shifting logR entries by the min value which was found while setting the values of logR
//		for(unsigned int j = 0; j < i; j++){
//			if (min > logR_new[i][j])
//				min = logR_new[i][j];
...
//		  for (unsigned int k=0; k < alignment.ncols() ;k++){
////			  if(k!=l){
//			  correction[l] += logR_new[l][k];
////			  }
//			  if (k < l){
//				  tot_average += logR_new[l][k];
//			  }
//		  }
//	    correction[l] /= num_non_diagonal;
//	  }
...
////	    	logR_new[k][l]-= (correction[l]*correction[k]) / tot_average;
//
//	      // rho part
//		  if (logR[k][l] > 0) {
//			  double temp = rho*exp(logR_new[k][l]) + (1.0 - rho);
//			  logR_new[k][l] = (temp);
//		  }	 else {
//			  double temp = log(rho) + logR_new[k][l] + log (1.0 + ((1.0 - rho)/rho)*exp(-logR_new[k][l]));
//			  logR_new[k][l] = exp(temp);
//		  }
//		  logR_new[l][k] = logR_new[k][l];
//	     }
//	  }
